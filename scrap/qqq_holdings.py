...
class QQQHoldingsScraper:
    ...
    def scrap(self):
        ...
        percents = [h.percentageOfTotalNetAssets for h in newHoldings]
    ...
    def data_filter(
        ...
        sorted_holdings = sorted(
            holdings, key=lambda h: h.percentageOfTotalNetAssets, reverse=True
        )
        ...
        for holding in sorted_holdings:
            if cumulative_percent < percent_threshold:
                ...
                cumulative_percent += holding.percentageOfTotalNetAssets
            else:
                other_holdings.append(holding)
        other_weight = sum(h.percentageOfTotalNetAssets for h in other_holdings)
        new_holdings.append(
            Holding(ticker="Other", percentageOfTotalNetAssets=other_weight)
        )
        return new_holdings
